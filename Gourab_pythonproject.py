...
while message:
    ...
    createfile('Students.csv',['Student ID','Name','Class Roll Number','Batch ID'])
...
def student():
    entries = int(input("Enter the number of students whose data you want to enter : "))
    
    for i in range(entries):
        name=(input("Enter the Name of the student : "))
        roll=(int(input("Enter the Class Roll Number : ")))
        batch=(input("Enter the batch which they are in (e.g. 2022-26) : "))
        stream=(input("Enter the stream to which they belong (e.g. CSE) : "))
        ...
        student_id=(batch_id+str(roll))
        batch_name=stream+' '+batch
        
        with open(f'{path}/Students.csv','a',newline='')as f:
         script= csv.writer(f)
        
         script.writerow([f"{student_id}",f"{name}",f"{roll}",f"{batch_id}"])
        ...
        createfile('Students.csv',[student_id,name,roll,batch_id])
        ...
        ask=input("Do you want to remove this name from database? If yes, now is the time (Y/N) : ")
...
def grade(num):
    if num>=90:
        return("Outstanding Performance... Your grade is A.")
    elif num<90 and num>=80:
        return("Excellent Performance... Your grade is B.")
    elif num<80 and num>=70:
        return("Good Performance... Your grade is C.")
    elif num<70 and num>=60:
        return("Your performance is average... Your grade is D.")
    elif num<60 and num>=50:
        return("Your performance is below average... Your grade is E.")
    else:
...
def remove(string):
    with open(f'C:/PythonProgrammingProject_main-folder/Students.csv','r+',newline='') as f:
        script=csv.reader(f)
        rows=[row for row in script]
        for i in rows:
            if i[0]==string:
